...
def rentsout(request):
    ...
    returned_rentsout = rentsout.filter(carNumber__status=2).first()
...
def carlist(request):
    ...
    rented_cars = Rent.objects.filter(~Q(carNumber__status=0)).filter(expired=0)
...
def cardetails(request, pk):
    teksti = None
    cartext = None
    image = CarImage.objects.filter(car__carNumber=pk).first()
